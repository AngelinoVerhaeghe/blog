...
@section('content')
    <section>
        ...
        <div class="flex justify-center bg-yellow-500 shadow-lg py-6">
            <h1 class="text-3xl text-gray-100 font-bold">Create Post</h1>
        </div>
            ...
            <div class="bg-white rounded-lg shadow-lg overflow-hidden pb-4 px-4 mt-15">
                <form action="/blogs" method="POST" enctype="multipart/form-data" class="space-y-6">
                    ...
                    <div>
                        ...
                        <input type="text" name="title" id="title" autocomplete="title" placeholder="Title..."
                            class="mt-1 focus:ring-yellow-500 focus:border-yellow-500 block w-full shadow-sm sm:text-sm border-gray-300 rounded-md @error('title') is-invalid @enderror"
                            value="{{ old('title') }}">
                        @error('title')
                    ...
                    <div>
                        ...
                        <div class="mt-1">
                            <textarea id="description" name="description" rows="10"
                                class="shadow-sm focus:ring-yellow-500 focus:border-yellow-500 mt-1 block w-full sm:text-sm border border-gray-300 rounded-md @error('description') is-invalid @enderror"
                                value="{{ old('description') }}" placeholder="Message..."></textarea>
                            @error('description')
                                <span class="text-red-500 text-xs italic mt-4">{{ $message }}</span>
                            @enderror
                        </div>
                    ...
                    <div class="px-4 py-3 border-t-2 border-gray-200 text-right space-x-3 mt-4">
                        ...
                        <button type="submit"
                            class="inline-flex justify-center py-2 px-4 border border-transparent shadow-sm text-sm font-medium rounded-3xl text-white bg-yellow-600 transition duration-300 ease-in-out hover:bg-yellow-700 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-yellow-500">
                            Save
                        </button>
                    </div>
                </form>
